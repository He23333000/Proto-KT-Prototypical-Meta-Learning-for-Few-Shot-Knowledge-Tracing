"""
Training script for Proto-KT meta-learning.
"""
import torch
import yaml
import argparse
...
def main(args):
    ...
    # Create Proto-KT model
    print(f"\nInitializing Proto-KT model...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Proto-KT")
    parser.add_argument('--data_path', type=str, required=True,
                        help='Path to processed dataset (.pkl)')
    parser.add_argument('--config', type=str, default='configs/config.yaml',
                        help='Path to config file')
    parser.add_argument('--num_prototypes', type=int, default=8,
                        help='Number of prototypes (k)')
    parser.add_argument('--save_dir', type=str, default='checkpoints/proto_kt',
